...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        [DataRow(false)]
        [DataTestMethod]
        public void TestSendAndCloseOnClosedSocket(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
            ...
            Assert.IsFalse(Networking.SendAndClose(testRemoteSocketState.TheSocket, message.ToString()));
        }

        [DataRow(true)]
        [DataRow(false)]
        [DataTestMethod]
        public void TestStopServer(bool clientSide)
        {
            SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
        ...
        {
            void badip(SocketState x)
            {
                testLocalSocketState = x;
            }
            Networking.ConnectToServer(badip, "1111111111111111111111111111111111111111111111111111111111", 2112);
            Assert.IsTrue(testLocalSocketState.ErrorOccurred);
        ...
        public void TestBadIP()
        {
            ...
            Networking.ConnectToServer(badip, "129.0.0.13", 2112);
            Assert.IsTrue(testLocalSocketState.ErrorOccurred);
